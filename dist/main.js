...
function seasonsOf(year) {
    const season = (id, from, to, dayFrom = to, dayTo = dayFrom) => ({
        id,
        from,
        to,
        dayFrom,
...
function seasonOn(date) {
    ...
    const ownDays = (s) => within(today, s.dayFrom, s.dayTo) ? s.dayTo.getTime() - s.dayFrom.getTime() : Infinity;
    let best = null;
    for (const s of seasonsAround(today)) {
        if (within(today, s.from, s.to) && (!best || ownDays(s) < ownDays(best)))
            best = s;
    }
    return best;
}
function nextSeason(date) {
    ...
    for (const s of seasonsAround(date)) {
        if (s.from.getTime() > date.getTime() && (!next || s.from.getTime() < next.from.getTime()))
            next = s;
    }
    return next;
}
...
function autoLabel() {
    ...
    if (current)
        return `${optionText(autoTheme)}: ${optionText(current)}`;
    const next = nextSeason(now);
    const upcoming = THEMES.find((t) => t.id === next?.id);
    if (!next || !upcoming)
        return optionText(autoTheme);
...
function saveTheme(theme) {
    ...
    try {
        ...
        if (shown.holiday)
            localStorage.setItem(HOLIDAY_KEY, shown.id);
        else
...
function showTheme(theme) {
    shownTheme = theme;
    if (theme.holiday)
        html.dataset.holiday = theme.id;
    else
        delete html.dataset.holiday;
    const greeting = theme.greeting ?? GREETING;
    if (heroTitle)
        heroTitle.textContent = greeting;
    const greetingBubble = chat.querySelector(".msg.greeting .bubble");
    if (greetingBubble)
...
if ((html.dataset.holiday ?? "") !== (shownTheme.holiday ? shownTheme.id : ""))
